...
# Load artifacts with caching; pastikan ini dijalankan sekali
@st.cache_resource
def load_artifacts():
    # Load model Keras dengan custom loss
    model = load_model(
        "ml_models/forecasting/model_global_region.h5",
        custom_objects={"mse": tf.keras.losses.MeanSquaredError()}
    )
    
    # Load scaler dan encoder dengan joblib (pastikan .joblib dibuat dengan numpy 1.24.3)
...
def run():
    ...
    if st.button("Jalankan Prediksi"):
        ...
        features_to_scale = ['day_of_year', 'month', 'year']
        scaled_features = scaler_temporal.transform(df_feat[features_to_scale])
        df_feat[features_to_scale] = scaled_features

        # Buat input sequence dengan kuantitas dummy = 0
        num_regions = len(regions)
        seq = np.zeros((1, look_back, num_regions), dtype=np.float32)  # pastikan float32
        seq[:, :, region_code] = 0.0

        # Prediksi dan inverse transform hasil prediksi
        ...
        # Tampilkan grafik dan tabel hasil prediksi
